...
Condition = 'if' space c:Flow space 'then' space s:Flow space 'else' space s:Flow space 'end' {
  return { type: 'condition', is: c, then: s, otherwise: s };
}
...
Function_WithLambdas
  ...
  / f:'unique_by' ws '(' ws a:Flow ws ')' { return mkfunc(f, lambda(a)); }
  / f:'while' ws '(' ws a:Flow ws ';' ws b:Flow ws ')' { return mkfunc(f, lambda(a), lambda(b)); }
  / f:'until' ws '(' ws a:Flow ws ';' ws b:Flow ws ')' { return mkfunc(f, lambda(a), lambda(b)); }
  / f:'recurse' ws '(' ws a:Flow ws ';' ws b:Flow ws ')' { return mkfunc(f, lambda(a), lambda(b)); }
  / f:'recurse' ws '(' ws a:Flow ws ')' { return mkfunc(f, lambda(a)); }
  / f:'walk' ws '(' ws a:Flow ws ')' { return mkfunc(f, lambda(a)); }
